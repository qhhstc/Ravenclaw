...
- 选择单周时，同一张 ROI 表增加本周/上周 ROI、变化值与变化率，保留本周销售额、广告费和广告占销；支持涨幅/跌幅优先排序。W1 使用上个月 W5（含跨年），按渠道 ID 匹配，缺数据不回退其他周。变化率为差值 ÷ |上周 ROI|；上周 ROI 为 0 时变化值仍可用，但变化率显示「—」。全月模式不显示周环比列，切回全月时重置仅适用于单周的涨跌排序。
