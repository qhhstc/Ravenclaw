    ...
    <Form.List name="items">
      {(fields, { add, remove }) => {
        ...
        const columns: ColumnsType<FieldRow> = [
          ...
          { title: "销售小计", width: 130, align: "right", render: (_, field) => <Form.Item shouldUpdate noStyle>{() => <div className="text-right font-medium text-[#172033]">{rowSubtotal(form, field.name, "saleUnitPrice")}</div>}</Form.Item> },
          ...
          { title: "采购小计", width: 130, align: "right", render: (_, field) => <Form.Item shouldUpdate noStyle>{() => <div className="text-right text-[#667085]">{rowSubtotal(form, field.name, "purchaseUnitCost")}</div>}</Form.Item> },
          ...
          { title: "包装小计", width: 130, align: "right", render: (_, field) => <Form.Item shouldUpdate noStyle>{() => <div className="text-right text-[#667085]">{rowSubtotal(form, field.name, "packagingUnitCost")}</div>}</Form.Item> },
          {
            title: "操作",
            width: 72,
            fixed: "right",
