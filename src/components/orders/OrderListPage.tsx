...
export default function OrderListPage() {
  ...
  const columns: ColumnsType<OrderRecord> = [
    ...
    { title: "销售总金额（订单币种）", dataIndex: "salesAmount", width: 170, align: "right", render: (value, row) => moneyText(value ?? row.totalAmount, row.currency) },
    { title: "总成本（本位币）", dataIndex: "totalCost", width: 150, align: "right", render: (value, row) => moneyText(value, row.baseCurrency || "CNY") },
    { title: "毛利（本位币）", dataIndex: "grossProfit", width: 150, align: "right", render: (value, row) => <span className={Number(value) < 0 ? "font-semibold text-red-500" : ""}>{moneyText(value, row.baseCurrency || "CNY")}</span> },
