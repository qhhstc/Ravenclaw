...
export default function OrderFormModal({ open, saving, editing, brands, platforms, stores, channels, influencers, countries, currencies, customers, users, products, canEditCosts = true, canEditPayments = canEditCosts, onCancel, onSubmit }: Props) {
    ...
    >
      ...
      >
        ...
        <div className="grid grid-cols-1 gap-x-4 md:grid-cols-3 xl:grid-cols-4">
          ...
          <Form.Item name="paymentMethod" label="收款方式"><Input disabled={!canEditPayments} placeholder="PayPal / 银行 / 平台" /></Form.Item>
          ...
          <Form.Item name="paymentStatus" label="付款状态" extra="默认按已收金额自动判断；手动选择后会按你的选择保存。"><Select disabled={!canEditPayments} options={paymentStatusOptions.map(({ label, value }) => ({ label, value }))} /></Form.Item>
          <Form.Item name="shippingStatus" label="发货状态"><Select options={shippingStatusOptions.map(({ label, value }) => ({ label, value }))} /></Form.Item>
          <Form.Item name="paidAmount" label="已收金额"><InputNumber disabled={!canEditPayments} min={0} precision={2} className="!w-full" /></Form.Item>
        ...
        {canEditCosts ? (
          ...
          <Alert type="info" showIcon message="收款由财务或管理员维护" description="业务员可以维护商品采购、包装及其他成本；收款金额、付款状态和收款方式由财务或管理员维护。" />
        )}
