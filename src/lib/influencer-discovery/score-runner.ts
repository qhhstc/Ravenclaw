...
export type ScoreChange = {
  item: Awaited<ReturnType<typeof prisma.influencerCandidate.update>>;
...
// 返回 old/new 分数与等级,供前端反馈。
export async function scoreCandidateById(candidateId: number, brandOverride?: WebsiteAnalysis): Promise<ScoreChange> {
  ...
  return {
    ...
    oldTier,
    newTier: result.tier,
    changed: oldScore !== result.score || oldTier !== result.tier,
  };
}
