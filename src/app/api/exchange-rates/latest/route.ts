...
export async function GET(request: NextRequest) {
  try {
    ...
    const asOfDate = dateParam ? new Date(dateParam) : null;
    const latestRate = await getLatestExchangeRate(from, to, { asOfDate: asOfDate && !Number.isNaN(asOfDate.getTime()) ? asOfDate : null });
